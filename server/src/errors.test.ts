...
// internals, credentials and SQL never reach a user.
...
describe("classifyUpstreamError", () => {
  test("recognises the capacity error that started all this", () => {
  ...
  describe("must NOT classify (falls through to a generic 500)", () => {
    ...
    test("a database error", () => {
      assert.equal(classifyUpstreamError(new Error('relation "public.Turn" does not exist')), null);
    });
...
describe("AppError", () => {
  test("defaults to a 400 that is safe to show", () => {
    const err = new AppError("Try a clearer photo.");
    assert.equal(err.statusCode, 400);
    assert.equal(err.code, "APP_ERROR");
    assert.equal(err.message, "Try a clearer photo.");
  });

  test("carries an explicit status and code", () => {
    const err = new AppError("Couldn't read any text in that photo.", {
      statusCode: 422,
      code: "NO_TEXT_IN_IMAGE",
    });
    assert.equal(err.statusCode, 422);
    assert.equal(err.code, "NO_TEXT_IN_IMAGE");
  });
