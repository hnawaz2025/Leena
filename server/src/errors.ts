...
// internals, SQL fragments and file paths -- and, in the one case that
// prompted this, told a user practising English that
// "zai-org/GLM-4-9B-0414 is temporarily at capacity", which is both alarming
// and useless to them.
  ...
  for (const link of errorChain(error)) {
    ...
    if (/quota|insufficient_quota|billing|payment required/i.test(text) || status === 402) {
      return {
        ...
        message: "The language service is temporarily unavailable. Please try again later.",
      };
    }

    // Capacity and rate limiting are transient and worth telling the user
    // about, because "try again shortly" is genuinely the right action.
    if (
      status === 429 ||
      status === 503 ||
      ...
      return {
        statusCode: 503,
        code: "AI_UNAVAILABLE",
        message: "The language service is busy right now. Please try again in a moment.",
      };
    }
